...
def test_detect_stale_values(stale_data):
    ...
    res1 = filtering.detect_stale_values(stale_data)
    res2 = filtering.detect_stale_values(stale_data, rtol=1e-8, window=2)
    res3 = filtering.detect_stale_values(stale_data, window=7)
    res4 = filtering.detect_stale_values(stale_data, window=8)
    res5 = filtering.detect_stale_values(stale_data, rtol=1e-8, window=4)
    res6 = filtering.detect_stale_values(stale_data[1:])
    res7 = filtering.detect_stale_values(stale_data[1:8])
...
def test_detect_stale_values_handles_negatives(data_with_negatives):
    ...
    res = filtering.detect_stale_values(data_with_negatives)
    assert_series_equal(res, pd.Series([False, False, True, True, False, False,
                                        False]))
    res = filtering.detect_stale_values(data_with_negatives, atol=1e-3)
    assert_series_equal(res, pd.Series([False, False, True, True, True, True,
                                        True]))
    res = filtering.detect_stale_values(data_with_negatives, atol=1e-5)
    assert_series_equal(res, pd.Series([False, False, True, True, True, False,
                                        False]))
    res = filtering.detect_stale_values(data_with_negatives, atol=2e-5)
    assert_series_equal(res, pd.Series([False, False, True, True, True, True,
                                        True]))
...
def test_detect_stale_values_raises_error(stale_data):
    ...
    with pytest.raises(ValueError):
        filtering.detect_stale_values(stale_data, window=1)
...
def test_detect_interpolation(interpolated_data):
    ...
    res1 = filtering.detect_interpolation(interpolated_data)
    assert_series_equal(res1, pd.Series([False, False, False, False, False,
                                         False, False, True, False, False,
                                         False, False, False, True, True, True,
                                         False]))
    res2 = filtering.detect_interpolation(interpolated_data, rtol=1e-2)
    assert_series_equal(res2, pd.Series([False, False, True, True, True,
                                         False, False, True, False, False,
                                         False, False, False, True, True, True,
                                         False]))
    res3 = filtering.detect_interpolation(interpolated_data, window=5)
    assert_series_equal(res3, pd.Series([False, False, False, False, False,
                                         False, False, False, False, False,
                                         False, False, False, False, False,
                                         True, False]))
    res4 = filtering.detect_interpolation(interpolated_data, atol=1e-2)
...
def test_detect_interpolation_handles_negatives(data_with_negatives):
    ...
    res = filtering.detect_interpolation(data_with_negatives, atol=1e-5)
    assert_series_equal(res, pd.Series([False, False, True, True, True, True,
                                        False]))
    res = filtering.detect_stale_values(data_with_negatives, atol=1e-4)
    assert_series_equal(res, pd.Series([False, False, True, True, True, True,
                                        True]))
...
def test_detect_interpolation_raises_error(interpolated_data):
    ...
    with pytest.raises(ValueError):
        filtering.detect_interpolation(interpolated_data, window=2)
