...
def test_run_pvwatts_data_checks_type_error(power_series, weather_df):
    ...
    with pytest.raises(TypeError,
                       match=('Power series must be a Pandas series with a ' +
                              'datetime index.')):
                           outliers.run_pvwatts_data_checks(
...
def test_run_pvwatts_data_checks_frequency(power_series, weather_df):
    ...
    """
    resampled_power_series = outliers.run_pvwatts_data_checks(power_series,
                                                              weather_df)
...
def test_run_pvwatts_model(serf_east_metadata, weather_df):
    ...
    tracking_predicted_power = outliers.run_pvwatts_model(tilt=tilt,
                                azimuth=azimuth,
                                dc_capacity=capacity,
                                dc_inverter_limit=capacity * 1.5,
    ...
    nontracking_predicted_power = outliers.run_pvwatts_model(tilt=tilt,
                                azimuth=azimuth,
                                dc_capacity=capacity,
                                dc_inverter_limit=capacity * 1.5,
                                solar_zenith=solpos.zenith,
...
def test_get_anomalous_days(serf_east_metadata, power_series, weather_df):
    """Test if dataframe is returned for systems without capacity.
    """
    lat, long, azimuth, tilt, tracking, _ = serf_east_metadata
    # Assume there's no capacity given
    master_df = outliers.get_anomalous_days(power_series, lat, long, tilt,
                                            azimuth, tracking, weather_df,
                                            pct_threshold=50,
                                            dc_capacity=None)
    master_df_freq = pd.infer_freq(master_df.index)
    # Assert that a dataframe is returned with datetime index
    assert isinstance (master_df.index, pd.DatetimeIndex)
    # Assert that expected column names are returned
    assert set(master_df.columns) == {"actual_power", "predicted_power",
                                      "percent_difference", "anomalous"}
    # Assert that dataframe has daily frequency
    assert master_df_freq == "D"
    # Assert flagged anomalous column is boolean
    assert master_df["anomalous"].dtype == bool
