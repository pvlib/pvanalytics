...
def test_detect_data_shifts(generate_daily_time_series):
    ...
    assert (abs((changepoint_date - shift_index_dates[0]).days) <= 7)
    assert (abs((changepoint_date - shift_index_unnamed_dates[0]).days) <= 7)
    assert (abs((changepoint_date - shift_index_param_dates[0]).days) <= 7)
